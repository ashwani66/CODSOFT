...
const ProductDetails = () => {
    ...
    const fetchCurrentProduct = async () => {
      try {
        ...
        if (currentProduct) {
          ...
          setMainImage(`${API}/${currentProduct.images[0]?.medium || currentProduct.images[0]?.small}`);
        }
      } catch (err) {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      setShowReviewForm(false); // Hide form after submit
        ...
        <div className="thumbnail-gallery">
          {product.images.map((imgObj, i) => (
            <img
              ...
              alt={`thumb-${i}`}
              onClick={() => setMainImage(`${API}/${imgObj.medium || imgObj.small}`)}
            />
          ))}
        </div>
      </div>
                ...
                <div className="options">
                  {[5,4,3,2,1].map(num => (
                    <div
                      key={num}
                      className="option"
                      onClick={() => { setNewReview({ ...newReview, rating: num }); setDropdownOpen(false); }}
                    >
                      {num} ⭐
                    </div>
                  ))}
      ...
      ⭐ Rate this Product
            ...
            reviews.map(r => (
              <div key={r._id} className="review-card">
                <p className="review-user">
                  <b>{r.userName || r.user?.username || "Anonymous"}</b> - {new Date(r.createdAt || r.date).toLocaleString()}
                </p>
        ...
        <div className="related-section">
          ...
          <div className="related-grid">
            {allProducts.filter(item => item._id !== product._id).map(item => (
              <Link key={item._id} to={`/products/${item._id}`} target="_blank" rel="noopener noreferrer" className="related-card">
                <img src={item.images?.[0]?.small ? `${API}/${item.images[0].small}` : "/images/no-image.png"} alt={item.name}/>
                <h4>{item.name}</h4>
                <p>₹{item.price}</p>
              </Link>
            ))}
